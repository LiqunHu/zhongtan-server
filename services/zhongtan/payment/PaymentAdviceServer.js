...
exports.searchAdminAct = async req => {
  ...
  if(search_data) {
    ...
    if (doc.search_data.payment_date && doc.search_data.payment_date.length > 1 && doc.search_data.payment_date[0] && doc.search_data.payment_date[1]) {
      queryStr += ` and payment_advice_id IN (
        SELECT
          payment_advice_id
        FROM
          tbl_zhongtan_payment_verification 
        WHERE
          state = 1 AND GREATEST(
            IFNULL( payment_verification_business_time, '1970-01-01 00:00:00' ),
            IFNULL( payment_verification_manager_time, '1970-01-01 00:00:00' ),
            IFNULL( payment_verification_undo_time, '1970-01-01 00:00:00' ),
            IFNULL( payment_verification_section_time, '1970-01-01 00:00:00' ),
          IFNULL( payment_verification_shipping_time, '1970-01-01 00:00:00' )) >= ? AND GREATEST(
            IFNULL( payment_verification_business_time, '1970-01-01 00:00:00' ),
            IFNULL( payment_verification_manager_time, '1970-01-01 00:00:00' ),
            IFNULL( payment_verification_undo_time, '1970-01-01 00:00:00' ),
            IFNULL( payment_verification_section_time, '1970-01-01 00:00:00' ),
          IFNULL( payment_verification_shipping_time, '1970-01-01 00:00:00' )) < ?
      ) `
...
exports.uploadAct = async req => {
  ...
  let iu = await tb_uploadfile.findOne({
        where: {
          api_name: 'PaymentAdviceServer_temporary',
          uploadfile_name: fileInfo.name,
          state: GLBConfig.ENABLE,
        }
      });
