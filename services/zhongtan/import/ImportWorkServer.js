...
exports.uploadImportAct = async req => {
  ...
  for (let f of doc.upload_files) {
    ...
    if(business_type === 'E') {
      ...
      let G_DATA_SEQ_ARRAY = []
      if(Array.isArray(xmldata.DATA_DS.G_DATA_SEQ)) {
        G_DATA_SEQ_ARRAY = xmldata.DATA_DS.G_DATA_SEQ
      } else {
        G_DATA_SEQ_ARRAY.push(xmldata.DATA_DS.G_DATA_SEQ)
      }
      for (let a of G_DATA_SEQ_ARRAY) {
        let blarray = []
...
exports.uploadAct = async req => {
  ...
  let iu = await tb_uploadfile.findOne({
        where: {
          api_name: 'ImportWorkServer_temporary',
