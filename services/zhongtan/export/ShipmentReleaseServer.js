...
exports.initAct = async () => {
  ...
  returnData.FIXED_RECEIVABLE_FEE = _.uniqBy(fixed_receivable_fee, 'fee_data_code')
...
exports.getBookingShipmentAct = async req => {
  ...
  if(bl) {
    ...
    if(ves.export_vessel_etd) {
      ...
      queryStr = `SELECT fee_data_code, fee_data_name, fee_data_enabled_start_date, fee_data_enabled_end_date FROM tbl_zhongtan_export_fee_data WHERE state = ? AND fee_data_payable = ? AND (fee_data_enabled_start_date IS NOT NULL OR fee_data_enabled_end_date IS NOT NULL) GROUP BY fee_data_code ORDER BY fee_data_id `
      replacements = [GLBConfig.ENABLE, GLBConfig.ENABLE]
          ...
          for(let s of sts) {
            if(shipment_fee_type === 'R') {
              ...
              if(con_fee_data) {
                ...
                if(export_masterbl_bl.indexOf('COSU') >= 0){
                  shipment_fee_party = con_fee_data.fee_data_payable_cosco_party ? con_fee_data.fee_data_payable_cosco_party : con_fee_data.fee_data_payable_common_party
                } else if(export_masterbl_bl.indexOf('OOLU') >= 0){
                  shipment_fee_party = con_fee_data.fee_data_payable_oocl_party ? con_fee_data.fee_data_payable_oocl_party : con_fee_data.fee_data_payable_common_party
                }
              }
            }
          }
